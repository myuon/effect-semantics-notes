...
places the result below the declared output.  For explicit `other`, use the
same argument with its tag; for default forwarding, rebuild precisely that
tag with the bare typed continuation.  A step inside the scrutinee follows
from Chapter-II preservation and handler-context replacement.  These are all
handler reduction rules, proving Theorem III.2.

## 3. Affine transformation

In the affine elaboration the matching clause is
...
The direct handler interpretation equals the first-boundary structural map.
:::
...
both sides apply $h_i(p,k)$.  For a nonmatching node, both sides rebuild the
same node with the original bare $k$.  Crucially no induction hypothesis is
used on $k$ in this last case.  These constructors are exhaustive. $\square$
