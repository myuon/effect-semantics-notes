...
The later trace-language development gives a precise reconciliation with the
displayed target $b e' e$: [Principal-word recovery v1](principal-word-recovery-v1.md)
proves an exact anchoring criterion and shows that the simple target is valid,
in particular, when the residual tail contains no later occurrence of the
handled interface.
