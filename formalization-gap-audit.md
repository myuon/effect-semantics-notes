...
| 6 | ordered effect upper-bound safety | `C1-MAIN.5.1` | **Checked and packaged** | `ProducesLanguageWriterTree.effectSound`, `LanguageWriterSemantics.gradeSound` |
| 7 | concrete base package extraction | `C1-MAIN.7.1`, `C1-MAIN.7.2` | **Finite semantic packages checked; typed graded bridge remains** | Writer/State/Exception each instantiate `FiniteBaseModel` with separate initial denotation, operational monad, comparison, and machine soundness; connecting generic operations to `LanguageSignature` grades in one dependent record remains |
...
| 11 | `HandlerPackage` / `ShallowHandlerPackage` packaging | `C3-MAIN.6.1`–`C3-MAIN.6.4` | **Checked and packaged** | `LanguageHandlerStage`, `languageHandlerStagePreservation`, `LanguageShallowMetatheory` |
...
| 15 | recursive/derived-deep package packaging | `C4-MAIN.1.1`, `C4-MAIN.7.1`, `C4-MAIN.7.2`, `C4-PROOF.6.1` | **Checked and packaged** | `LanguageRecursiveTheory`, `languageRecursiveStructurePreservation`, plus generic recursive morphism/relation/TT packages |
