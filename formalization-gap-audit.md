...
| 4 | recursion-free normalization and reducibility | `C1-CERT.3.1`, `C1-PROOF.4.1`, `C1-PROOF.4.2` | **Precisely scoped; grammar checked** | `FinLanguageVal`/`FinLanguageComp` exclude `fixLam` by construction; renaming, substitution and step simulation are checked; logical reducibility plus a well-founded response kernel remain |
...
| 7 | concrete base certificate extraction | `C1-CERT.7.1`, `C1-CERT.7.2` | **Repaired boundary** | Writer/State/Exception generic structures checked; Random/SubDist removed from checked extraction claim |
