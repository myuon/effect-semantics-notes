...
| 3 | canonical forms and unique selected position | `C1-CERT.2.2`, `C1-PROOF.3.1`, `C1-PROOF.3.2`, `C2-CERT.2.1` | **Existing, including direct Chapter-II four-way form** | `closed_bool_canonical`, `closed_arr_canonical`, `closed_sum_canonical`, `LanguageProgress.kind_unique`, `LanguageStep.deterministic`, `progressClosed_fourWayExactlyOne` |
...
| 9 | source-tree/generic bridge and graded `FreeCert` boundary | `C2-CERT.5.1`, `C2-CERT.8.1`–`C2-CERT.8.3` | **Finite bridge checked; graded identification remains conditional** | `LanguageWriterTree.toFreeExtension_bind` connects the finite source tree to `FreeExtension`; exact source `LanguageFreeStageCert` and generic `GenericFreeExtensionCert`; $F_eA$ still requires the paper `StrongGradedFreeT` hypothesis |
