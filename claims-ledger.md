...
| C-259 | Conjecture revised | An affine shallow match admits the sound upper-bound transformation $b\cdot\Delta\cdot e\mapsto b\cdot e'\cdot e$ and lifts the finite base adequacy certificate | C-267; finite free-tree model | prove Writer, State and Exception instances |
...
| C-277 | Derived on paper | Under $1\leq e'$, an affine response handler soundly maps $b\cdot\Delta\cdot e$ to $b\cdot e'\cdot e$, including executions where the optional $\Delta$ does not occur | residual-context typing; monotonicity | independently audit no-operation branch |
...
| C-279 | Derived conditional | Direct shallow reduction commutes with structural first-boundary denotation and preserves finite ground adequacy | `FreeCert`; related clauses; constructor separation | expand nested handler cases |
...
| C-281 | Adopted definition | Deep handling is derived solely by a recursive wrapper around Chapter-III matching and forwarding continuations | `ShallowCert`; computation-level fixpoint | audit source elaboration |
